...
async def extract_session_id(request: Request, body_json: dict = None) -> str:
    ...
    for header in ["X-Request-ID", "X-Correlation-ID", "X-Conversation-ID", "X-Session-ID"]:
...
async def proxy_v1_request(path: str, request: Request):
    ...
    logger.debug(f"Session {session_id} | Full Request Headers: {dict(request.headers)}")
    if body_json:
        logger.debug(f"Session {session_id} | Full Request Body (JSON): {json.dumps(body_json, ensure_ascii=False)}")
    elif content:
        logger.debug(f"Session {session_id} | Full Request Body (Raw): {content.decode('utf-8', errors='replace')}")
    ...
    try:
        ...
        async def response_stream_wrapper():
            ...
            logger.debug(f"Session {session_id} | Full Response Headers: {response_data['headers']}")
            if isinstance(response_body_json, dict):
                logger.debug(f"Session {session_id} | Full Response Body (JSON): {json.dumps(response_body_json, ensure_ascii=False)}")
            else:
                logger.debug(f"Session {session_id} | Full Response Body (Raw): {response_body_json}")
